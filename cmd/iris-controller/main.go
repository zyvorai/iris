...
func main() {
	dbPath := env("IRIS_DB_PATH", "/data/iris/iris.db")
	publicBase := env("IRIS_PUBLIC_BASE_URL", "https://localhost:31847")
	publicPathPrefix := env("IRIS_PUBLIC_PATH_PREFIX", "")
	autoPublish := env("IRIS_AUTO_PUBLISH", "false") == "true"
	autoSuggest := env("IRIS_AUTO_SUGGEST", "true") != "false"
	discoverAll := env("IRIS_DISCOVER_ALL", "true") != "false"
	discoverIngress := env("IRIS_DISCOVER_INGRESS", "true") != "false"
	discoverGatewayAPI := env("IRIS_DISCOVER_GATEWAY_API", "true") != "false"
	discoverMesh := env("IRIS_DISCOVER_MESH", "true") != "false"
	watchNS := splitCSV(env("IRIS_WATCH_NAMESPACES", ""))
