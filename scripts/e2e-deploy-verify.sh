...
check "launchpad gateway route" "curl -sf -o /dev/null -w '%{http_code}' '${BASE}/launchpad/a/hermes-demo/grafana' | grep -qE '^(200|302|307|308)$'"
check "launchpad canonical route" "code=\$(curl -s -o /dev/null -w '%{http_code}' '${BASE}/launchpad/apps/grafana'); test \"\$code\" != '404'"
check "legacy gateway route" "curl -sf -o /dev/null -w '%{http_code}' '${BASE}/a/hermes-demo/grafana' | grep -qE '^(200|302|307|308)$'"
