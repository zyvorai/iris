...
check "workspaces API" "curl -sf '${BASE}/api/v1/workspaces'"
check "owners API" "curl -sf '${BASE}/api/v1/owners'"
check "search API" "curl -sf '${BASE}/api/v1/search?q=gra'"
check "search intent API" "curl -sf '${BASE}/api/v1/search/intent?q=unhealthy' | grep -q '\"intent\"'"
check "federated catalog API" "curl -sf '${BASE}/api/v1/catalog/federated' | grep -q '\"clusterId\"'"
check "federation write API" "curl -sf -X POST '${BASE}/api/v1/federation/publish/_missing_/demo/app' | grep -q '\"ok\"' || test \"\$(curl -s -o /dev/null -w '%{http_code}' -X POST '${BASE}/api/v1/federation/publish/_missing_/demo/app')\" = '404'"
check "federation rbac API" "curl -sf '${BASE}/api/v1/federation/rbac/_missing_' >/dev/null 2>&1 || test \"\$(curl -s -o /dev/null -w '%{http_code}' '${BASE}/api/v1/federation/rbac/_missing_')\" = '404'"
check "federated audit API" "curl -sf '${BASE}/api/v1/audit/federated?limit=5' | grep -q '\"clusterId\"'"
check "search llm API" "curl -sf '${BASE}/api/v1/search/llm?q=grafana' | grep -q '\"intent\"'"
check "clusters API" "curl -sf '${BASE}/api/v1/clusters' | grep -q '\"status\"'"
check "shares admin API" "curl -sf '${BASE}/api/v1/shares/all' | grep -q '^\\['"
