...
async fn proxy_app(st: GatewayState, app: iris_core::App, rest: String, req: Request<Body>) -> Response {
    if !app.visibility.published {
        return (StatusCode::FORBIDDEN, "app not published").into_response();
    }
...
struct ForwardContext {
    ...
    iris_user: Option<String>,
}

fn collect_forward_context(headers: &HeaderMap, app: &iris_core::App) -> ForwardContext {
    // Iris terminates TLS itself (rustls); default to https so backends
...
async fn http_proxy(app: iris_core::App, rest: String, req: Request<Body>) -> Response {
    ...
    inject_iris_headers(&mut headers, &app, &forward);
...
fn resolve_app_from_referer(st: &GatewayState, referer: &str) -> Option<iris_core::App> {
...
/// public mount so the browser hits the gateway again instead of the Iris SPA.
pub fn rewrite_mode_a_body(app: &iris_core::App, content_type: Option<&str>, body: Bytes) -> Bytes {
...
fn inject_mount_shim(mount: &str, html: &str) -> String {
    if html.contains("data-iris-mount-shim") {
        return html.to_string();
    }
    let mount_js = mount.replace('\\', "\\\\").replace('\'', "\\'");
    let shim = format!(
        r#"<script data-iris-mount-shim>(function(m){{
if(typeof window==='undefined'||window.__irisMountShim)return;window.__irisMountShim=m;
...
fn rewrite_forward_query(app: &iris_core::App, query: Option<&str>) -> Option<String> {
    if app.rewrite.add_prefix.is_empty() {
        let mount = route_mount(app);
        if let Some(q) = query {
            if q == "next=%2F" || q == "next=/" {
...
pub fn rewrite_root_absolute_refs(mount: &str, input: &str) -> String {
    ...
    // App APIs under /api/… — leave Iris control-plane /api/v1 alone.
...
async fn tunnel_websocket(
    ...
    let mut request = upstream_url.into_client_request()?;
    {
        ...
        if let Some(user) = &forward.iris_user {
            let _ = headers.insert(
                "x-iris-user",
...
pub fn build_backend_path(app: &iris_core::App, rest: &str) -> String {
...
fn inject_iris_headers(headers: &mut HeaderMap, app: &iris_core::App, forward: &ForwardContext) {
    headers.insert(
        HeaderName::from_static("x-iris-app"),
        HeaderValue::from_str(&app.slug).unwrap_or(HeaderValue::from_static("unknown")),
    );
    headers.insert(
        HeaderName::from_static("x-iris-namespace"),
    ...
    if app.auth_mode != "none" {
        if let Some(user) = forward.iris_user.as_deref() {
            let _ = headers.insert(
...
fn route_mount(app: &iris_core::App) -> String {
...
pub fn rewrite_location(app: &iris_core::App, location: &str) -> String {
    let mount = route_mount(app);
...
fn rewrite_absolute_path(app: &iris_core::App, mount: &str, path: &str) -> String {
    if path.is_empty() || path == "/" {
        return format!("{mount}/");
    }
...
fn rewrite_redirect_header(app: &iris_core::App, name: &str, raw: &[u8]) -> Option<HeaderValue> {
...
fn rewrite_refresh(app: &iris_core::App, value: &str) -> String {
...
pub fn rewrite_set_cookie_value(app: &iris_core::App, value: &str) -> String {
    let mount = route_mount(app);
    if mount.is_empty() {
...
fn rewrite_set_cookie(app: &iris_core::App, raw: &[u8]) -> Option<HeaderValue> {
    let s = std::str::from_utf8(raw).ok()?;
...
mod tests {
    use super::*;
    use iris_core::{App, AppMeta, Backend, Rewrite, Visibility};

    fn sample_app(rewrite: Rewrite) -> App {
        App {
            id: "iris-demo/grafana".into(),
            slug: "grafana".into(),
            ...
            route_path: "/launchpad/a/iris-demo/grafana".into(),
    ...
    fn backend_path_mode_a_root_and_rest() {
        let app = sample_app(Rewrite {
            strip_prefix: "/launchpad/a/iris-demo/grafana".into(),
    ...
    fn backend_path_mode_b_add_prefix() {
        let app = sample_app(Rewrite {
            ...
            add_prefix: "/launchpad/a/iris-demo/grafana".into(),
        });
        assert_eq!(
            build_backend_path(&app, ""),
            "/launchpad/a/iris-demo/grafana/"
        );
        assert_eq!(
            build_backend_path(&app, "login"),
            "/launchpad/a/iris-demo/grafana/login"
        );
        assert_eq!(
            build_backend_path(&app, "/api/health"),
            "/launchpad/a/iris-demo/grafana/api/health"
        );
    }
    ...
    fn location_rewrite_absolute_paths() {
        ...
        assert_eq!(
            rewrite_location(&app, "/launchpad/a/iris-demo/grafana/"),
            "/launchpad/a/iris-demo/grafana/"
        );
        assert_eq!(
            rewrite_location(&app, "http://grafana.iris-demo.svc/login"),
            "/launchpad/a/iris-demo/grafana/login"
        );
    }
    ...
    fn set_cookie_path_scoped_to_mount() {
        ...
        assert_eq!(
            ...
            "sid=abc; Path=/launchpad/a/iris-demo/grafana/; HttpOnly"
        );
        assert_eq!(
            rewrite_set_cookie_value(&app, "sid=abc; Path=/grafana"),
            "sid=abc; Path=/launchpad/a/iris-demo/grafana/grafana"
        );
    }
    ...
    #[test]
    fn mode_a_body_skips_already_mounted_and_iris_api() {
        let mount = "/launchpad/a/zyvor-janus/zyvor-janus-web";
        let html = format!(
    ...
    fn mode_a_html_injects_mount_shim() {
        ...
        assert!(s.contains("data-iris-mount-shim"));
        assert!(s.contains("/launchpad/a/iris-demo/grafana"));
    }
}
