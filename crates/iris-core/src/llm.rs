...
pub fn llm_config_from_env() -> Option<LlmConfig> {
    let api_url = std::env::var("IRIS_LLM_API_URL").ok()?;
    if api_url.trim().is_empty() {
        return None;
    }
    Some(LlmConfig {
        api_url,
        api_key: std::env::var("IRIS_LLM_API_KEY").unwrap_or_default(),
        model: std::env::var("IRIS_LLM_MODEL").unwrap_or_else(|_| "gpt-4o-mini".into()),
    })
}
...
pub async fn llm_search_intent(apps: &[App], query: &str, cfg: &LlmConfig) -> Option<SearchIntent> {
    let catalog: Vec<_> = apps
        ...
        .collect();
    let system = "You are Iris app search. Return JSON only: {\"intent\":\"...\",\"answer\":\"...\",\"appIds\":[\"id1\"]}. Pick matching app ids from the catalog.";
    let user = format!(
        "Catalog: {}\n\nQuery: {}",
