...
pub fn federated_cluster_configs() -> Vec<FederatedClusterConfig> {
    std::env::var("IRIS_FEDERATED_CLUSTERS")
        .ok()
...
pub fn local_cluster(apps: &[App]) -> ClusterInfo {
    let id = std::env::var("IRIS_CLUSTER_ID").unwrap_or_else(|_| "local".into());
    let name = std::env::var("IRIS_CLUSTER_NAME").unwrap_or_else(|_| "Local cluster".into());
