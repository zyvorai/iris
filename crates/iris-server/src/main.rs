...
use iris_api::ApiState;
use iris_core::store::Store;
use iris_gateway::GatewayState;
use tower_http::{
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "iris_server=info,tower_http=info".into()),
        )
        .init();
    ...
    let bind = env::var("IRIS_BIND").unwrap_or_else(|_| "0.0.0.0:31847".into());
    let db_path = env::var("IRIS_DB_PATH").unwrap_or_else(|_| "/data/iris/iris.db".into());
    let tls_cert = env::var("IRIS_TLS_CERT").ok();
    let tls_key = env::var("IRIS_TLS_KEY").ok();
    let tls_dir = env::var("IRIS_TLS_DIR").unwrap_or_else(|_| {
        Path::new(&db_path)
            .parent()
            .map(|p| p.join("tls"))
            .unwrap_or_else(|| PathBuf::from("/data/iris/tls"))
            .to_string_lossy()
            .into_owned()
    });
    let tls_san_hosts = auth::split_csv(&env::var("IRIS_TLS_SAN_HOSTS").unwrap_or_default());
    let ui_dir = env::var("IRIS_UI_DIR").unwrap_or_else(|_| "./ui/dist".into());
    let default_user = env::var("IRIS_DEFAULT_USER").unwrap_or_else(|_| "local".into());
    let allowed_namespaces = auth::split_csv(&env::var("IRIS_ALLOWED_NAMESPACES").unwrap_or_default());
    let admin_users = auth::split_csv(&env::var("IRIS_ADMIN_USERS").unwrap_or_default());
    let admin_groups = auth::split_csv(&env::var("IRIS_ADMIN_GROUPS").unwrap_or_default());
    let workspace_rules = iris_core::workspace_acl::workspace_rules_from_env();
    let role_rules = iris_core::rbac::role_rules_from_env();
    ...
    let api = iris_api::routes(api_state);
    let gateway = iris_gateway::routes(gateway_state);
    ...
    tracing::info!("iris-server listening on https://{addr} (auth={})", auth_cfg.mode);
    axum_server::bind_rustls(addr, tls_config)
        .handle(handle)
        .serve(app.into_make_service())
        .await?;
    tracing::info!("iris-server stopped");
    Ok(())
}

/// When a Mode A launchpad app navigates to a root-absolute path (e.g. `/simulate`),
/// bounce back under the Referer mount instead of serving Iris 404.
...
fn is_iris_dock_path(path: &str) -> bool {
    if path == "/" {
        return true;
    }
    // Exact segments / prefixes for Iris Dock — avoid bare "/s" matching "/simulate".
