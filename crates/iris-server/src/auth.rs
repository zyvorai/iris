...
const SESSION_COOKIE: &str = "iris_session";
...
impl AuthConfig {
    pub fn from_env(fallback_user: String) -> Result<Self> {
        const DEV_SECRET: &str = "iris-dev-session-secret-change-me";
        let session_secret = std::env::var("IRIS_SESSION_SECRET")
            .unwrap_or_else(|_| DEV_SECRET.into());
        if session_secret == DEV_SECRET {
            tracing::warn!(
                "IRIS_SESSION_SECRET is using the insecure development default. \
                 Set it to a random secret (e.g. `openssl rand -hex 32`) before deploying."
            );
        }
        Ok(Self {
            mode: std::env::var("IRIS_AUTH_MODE").unwrap_or_else(|_| "none".into()),
            api_key: std::env::var("IRIS_API_KEY").unwrap_or_default(),
            fallback_user,
            session_secret,
            oidc_issuer: std::env::var("IRIS_OIDC_ISSUER").unwrap_or_default(),
            oidc_client_id: std::env::var("IRIS_OIDC_CLIENT_ID").unwrap_or_default(),
            oidc_client_secret: std::env::var("IRIS_OIDC_CLIENT_SECRET").unwrap_or_default(),
            oidc_redirect_url: std::env::var("IRIS_OIDC_REDIRECT_URL").unwrap_or_default(),
...
pub async fn require_auth(
    ...
    if let Some(user) = user_from_session(&auth, &jar) {
        ...
        let _ = req.headers_mut().insert(
            HeaderName::from_static("x-iris-user"),
            HeaderValue::from_str(&user).unwrap_or(HeaderValue::from_static("local")),
        );
        if !groups.is_empty() {
            let _ = req.headers_mut().insert(
                HeaderName::from_static("x-iris-groups"),
...
async fn login(State(auth): State<AuthConfig>) -> Result<Response, AppAuthError> {
    ...
    let return_to = Cookie::build(("iris_auth_return", "/"))
        .path("/")
        .http_only(true)
        .secure(true)
...
async fn me(State(auth): State<AuthConfig>, headers: HeaderMap) -> Json<AuthMe> {
    ...
    let rules = iris_core::workspace_acl::workspace_rules_from_env();
    let role_rules = iris_core::rbac::role_rules_from_env();
    let admin_users = split_csv(&std::env::var("IRIS_ADMIN_USERS").unwrap_or_default());
    let admin_groups = split_csv(&std::env::var("IRIS_ADMIN_GROUPS").unwrap_or_default());
    ...
    Json(AuthMe {
        ...
        groups: groups.clone(),
        allowed_workspaces: iris_core::workspace_acl::allowed_workspaces(&groups, &rules),
        allowed_actions: iris_core::rbac::allowed_actions_for_groups(&groups, &role_rules, is_admin),
    })
}
...
async fn fetch_discovery(issuer: &str) -> Result<OidcDiscovery> {
    if issuer.is_empty() {
        return Err(anyhow!("IRIS_OIDC_ISSUER is required"));
    }
    let url = format!(
...
async fn exchange_code(
    ...
    if auth.oidc_client_id.is_empty() || auth.oidc_redirect_url.is_empty() {
        return Err(anyhow!("IRIS_OIDC_CLIENT_ID and IRIS_OIDC_REDIRECT_URL are required"));
    }
    let mut form = vec![
...
fn federation_trust_headers() -> bool {
    std::env::var("IRIS_FEDERATION_TRUST_HEADERS")
