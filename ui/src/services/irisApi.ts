...
export const irisApi = {
  listApps: () => req<IrisApp[]>('/apps'),
  listCatalog: () => req<IrisApp[]>('/catalog'),
  ...
  getApp: (id: string) => req<IrisApp>(appPath(id)),
  ...
  listDiscovery: () => req<IrisApp[]>('/discovery'),
  ...
  listFavorites: () => req<IrisApp[]>('/favorites'),
...
export function appDetailPath(app: IrisApp, diagnose = false): string {
  const base = `/apps/${encodeURIComponent(app.id)}`
  return diagnose ? `${base}?diagnose=1` : base
}

export function copyAppUrl(app: IrisApp) {
...
export function appLaunchPath(app: IrisApp): string {
...
export function appPublicUrl(app: IrisApp): string {
...
export async function openApp(app: IrisApp) {
  ...
  void irisApi.recordRecent(launchApp.id)
